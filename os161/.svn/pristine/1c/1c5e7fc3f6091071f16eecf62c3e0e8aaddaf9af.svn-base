...
thread_create(const char *name)
{
	...
	//kprintf("Above while\n");
	while(process_table[i].pid!=-1){
		if(i<P_TABLE_SIZE-1)
			i++;		
		else
			return EAGAIN;
	}
	//kprintf("Bellow while\n");

	process_table[i].pid=i;
	process_table[i].status = 1;
	process_table[i].exitCode = 0;
	process_table[i].numOfChild = 0;

	thread->pid = i;

	//kprintf("THREAD PID: %d\n", thread->pid);
	splx(spl);	
...
thread_bootstrap(void)
{
	...
	for(i=1;i<MAX_NUM_CHILD;i++){
		if(process_table[curthread->pid].childIDs[i]==0){
		break;
		}
	} 
	process_table[curthread->pid].childIDs[i]=me->pid;
	process_table[curthread->pid].childIDs[process_table[curthread->pid].numOfChild]=newguy->pid;
	process_table[curthread->pid].numOfChild++;
	*/


	/////// 1 thread
	me->pid = 1;
	process_table[me->pid].pid = 1;
	process_table[me->pid].numOfChild += 1;
	int i;
	for(i=1;i<MAX_NUM_CHILD;i++){
		if(process_table[me->pid].childIDs[i]==0){
		break;
		}
	} 
	process_table[me->pid].childIDs[i]=me->pid;
	process_table[me->pid].childIDs[process_table[me->pid].numOfChild]=me->pid;	
	process_table[me->pid].numOfChild++;
	/*
...
int initialize_Process_Table(){//-----------------------------------------------------------------------------------------------------------------------------initialize_Process_Table
	process_table = (struct process_info *)kmalloc(sizeof(struct process_info)*P_TABLE_SIZE);
	int i;
	//kprintf("Entering initialize_process_table");
	for(i=0;i<P_TABLE_SIZE;i++){
		process_table[i].pid = -1;
		process_table[i].status = 0;
		process_table[i].exitCode = 0;
	 	process_table[i].numOfChild = 0;
		process_table[i].childIDs = kmalloc(sizeof(int)*MAX_NUM_CHILD);
		process_table[i].has_been_waited = 0;
		int j;
		for(j=0;j<MAX_NUM_CHILD;j++){
			process_table[i].childIDs[j]=0;
		}
	}
...
struct thread *newguy;
	...
	if(process_table[curthread->pid].pid==-1){
		kprintf("WE HAVE A PROBLEM HERE IN thread.c: thread_fork()\n");
	}

	//record the child this process forked (for ASST2 only one child per process?)
	int i;
	...
	process_table[curthread->pid].childIDs[i]=newguy->pid;
	

	//kprintf("new guy pid: %d\n", newguy->pid);

	//kprintf("Out of thread_create\n");
	...
	if (curthread->v != NULL) {
		VOP_INCREF(curthread->v);
		VOP_INCOPEN(curthread->v);
		newguy->v = curthread->v;
	}
	 ...
	 */
	numthreads++;
	//kprintf("NEW GUY PID 1: %d\n", newguy->pid);
	/* Done with stuff that needs to be atomic */
	splx(s);
	//kprintf("NEW GUY PID 2 : %d\n", newguy->pid);
	/*
	...
	//kprintf("NEW GUY PID 3: %d\n", newguy->pid);
	//kprintf("Out of thread_fork\n");
...
{//---------------------------------------------------------------------------------------------------------------------------------------thread_exit
	...
	/*update the information in process_info table*/
		process_table[curthread->pid].status = 2;
	...
	if (curthread->v != NULL){
		if(curthread->v->vn_refcount == 1){
			vfs_close(curthread->v);
		} else {
			VOP_DECREF(curthread->v);
			VOP_DECOPEN(curthread->v);
		}
	}
