...
//----------------------------------------------------------------------------------
    ...
    result = thread_fork(args[0] /* thread name */,
            ...
            cmd_progthread, &t_temp);

	//runprogram(args[0], args, nargs);
	...
	int status;
	int retval;
	waitpid(t_temp->pid, &status, 0, &retval);
...
cmd_dbflags(int n, char **a)//--------------------------------------------------------------------------------------------------
{
    ...
    showmenu("OS/161 dbflags menu", dbflagsmenu);
...
static
int
dfmodify(int n, char**a) {

    char* option = a[1];
    char* on_off = a[2];
    int correctnessInput = 0; //used as boolean
...
static struct {
    ...
    /* lab0: dbflags modification command*/
    { "df", dfmodify},
